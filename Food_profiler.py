...
def _resolve_axis_policy(axes_def: dict, matched_category: str | None,
                          keyword: str) -> dict:
    """
    axis_policy 결정. lookup 우선순위:
      1) matched_category (예: '커피') 가 axis_policy에 있으면 그것
      2) keyword 자체가 axis_policy 키이면 그것
      3) _default
    누락된 필드는 _default로 채움.
    """
    policy_block = (axes_def.get("axis_policy") or {})
    ...
    chosen = None
    if matched_category and matched_category in policy_block:
        chosen = policy_block[matched_category]
    elif keyword in policy_block:
        chosen = policy_block[keyword]
...
def compose_axes(keyword: str) -> tuple[dict, dict]:
    ...
    # ── matched category 결정 (axis_policy lookup 전에 먼저) ──
    ...
    # ── axis_policy 결정 ──
    policy = _resolve_axis_policy(axes_def, matched, keyword)
    ...
    # ── food_specific 축 ──
    if matched and use_food_specific:
        print(f"  📋 '{keyword}' → '{matched}' 카테고리 "
              f"(shared={'all' if allow_shared else (list(shared_allowlist) or 'none')})")
        ...
        print(f"  📋 '{keyword}' → 공용 축"
              f" ({'전체' if allow_shared else 'allowlist만'})")
