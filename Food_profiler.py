"""
Food Profiler v4.1 — Stand JSON 완전 연동 (버그 수정)
======================================================
수정사항 (v4.1):
  - build_profiles에 filtered 데이터를 넣도록 수정 (df → filtered)
  - keyword_reviews 카운트에 aliases 반영
  - 키워드 관련 리뷰가 2건 미만인 식당 제외
  - avg_total NaN 방지

실행: python Food_profiler.py 순대국밥
"""
...
def compose_axes(keyword):
    ...
    def build_entry(axis_name, group, lex_section, lex_key=None):
        lex = lex_section.get(lex_key or axis_name, {})
        return {"positive": lex.get("positive", []), "negative": lex.get("negative", []),
                "group": group, "clip_prompt_pos": lex.get("clip_prompt_pos", []),
                "clip_prompt_neg": lex.get("clip_prompt_neg", [])}

    meta = {}
    for name, info in axes_def.get("meta_axes", {}).items():
        if name.startswith("_"): continue
        meta[name] = build_entry(name, info.get("group", "메타"), lexicon.get("common", {}))
    ...
    if matched:
        ...
        for name, info in food_specific[matched].items():
            taste[name] = build_entry(name, info.get("group", matched + "특화"), spec_lex)
    else:
        print(f"  📋 '{keyword}' → 카테고리 없음 → 공용 전체 사용")
    return taste, meta

def _fallback_axes():
    taste = {"매운맛": {"positive":["매운","매콤","얼큰"],"negative":["안 매","순한"],"group":"맛","clip_prompt_pos":[],"clip_prompt_neg":[]},
             "바삭함": {"positive":["바삭","겉바","크리스피"],"negative":["눅눅"],"group":"식감","clip_prompt_pos":[],"clip_prompt_neg":[]}}
    meta = {"전반적만족": {"positive":["맛있","맛나","최고"],"negative":["맛없","별로"],"group":"메타","_weight":0.3,"clip_prompt_pos":[],"clip_prompt_neg":[]}}
    return taste, meta
    ...
    for axis_name, axis_info in all_axes.items():
        for kw in axis_info.get("positive", []):
            if kw in text:
                ...
                if axis_name not in meta_axes: has_specific = True
...
def load_csvs():
    ...
    if not frames:
        print("  ERROR: 유효한 CSV가 없습니다!"); return None
...
def build_profiles(filtered_df, keyword, taste_axes, meta_axes, rules):
    """★ filtered_df만 받음 — 키워드 관련 리뷰만"""
    all_axes = {**taste_axes, **meta_axes}
    ...
    for restaurant in filtered_df["Restaurant"].unique():
        sub = filtered_df[filtered_df["Restaurant"] == restaurant]
        kw_count = len(sub)
        if kw_count < 2: continue  # 최소 2건
        ...
        normalized = {a: round(sum_scores[a] / scored_count, 4) for a in axes_names} if scored_count > 0 else {a: 0.0 for a in axes_names}
        avg_total = round(float(sub["Total"].mean()), 2) if "Total" in sub.columns and sub["Total"].notna().any() else 0
        avg_taste = round(float(sub["Taste"].mean()), 2) if "Taste" in sub.columns and sub["Taste"].notna().any() else 0

        profiles[restaurant] = {
            "vector": [normalized[a] for a in axes_names], "normalized": normalized,
            "taste_vector": {a: normalized[a] for a in taste_names},
            "meta_vector": {a: normalized[a] for a in meta_names},
            "evidence": evidence, "total_reviews": kw_count,
            "keyword_reviews": kw_count, "scored_reviews": scored_count,
            "avg_total": avg_total, "avg_taste": avg_taste,
        }
...
def save_results(keyword, profiles, axes_names, all_axes, taste_axes, meta_axes):
    ...
    for name, p in profiles.items():
        ...
        for a in axes_names: row[a] = p["normalized"][a]
        rows.append(row)
    ...
    output = {
        "keyword": keyword, "axes": axes_names,
        "taste_axes": list(taste_axes.keys()), "meta_axes": list(meta_axes.keys()),
        "groups": dict(groups),
        "axes_config": {name: {"group": info.get("group","기타"), "positive_keywords": info.get("positive",[]),
            "negative_keywords": info.get("negative",[]), "clip_prompt_pos": info.get("clip_prompt_pos",[]),
            "clip_prompt_neg": info.get("clip_prompt_neg",[]), "is_meta": name in meta_axes}
            for name, info in all_axes.items()},
        "restaurants": {name: {"vector": p["vector"], "normalized": p["normalized"],
            "taste_vector": p["taste_vector"], "meta_vector": p["meta_vector"],
            "evidence": p["evidence"], "total_reviews": p["total_reviews"],
            "keyword_reviews": p["keyword_reviews"], "avg_total": p["avg_total"], "avg_taste": p["avg_taste"]}
            for name, p in profiles.items()},
    }
    with open(f"{safe}_vectors.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    cfg = {name: {"group": info.get("group","기타"), "keywords": info.get("positive",[]),
        "clip_prompt_pos": info.get("clip_prompt_pos",[]), "clip_prompt_neg": info.get("clip_prompt_neg",[])}
        for name, info in all_axes.items()}
    with open(f"{safe}_axes_config.json", "w", encoding="utf-8") as f:
        json.dump(cfg, f, ensure_ascii=False, indent=2)
    print(f"  ✅ {safe}_dictionary.csv")
    print(f"  ✅ {safe}_vectors.json")
    print(f"  ✅ {safe}_axes_config.json")

def main():
    if len(sys.argv) > 1: keyword = sys.argv[1]
    else: keyword = input("분석할 음식 키워드 (예: 짜장면, 삼겹살): ").strip()
    ...
    if kw_count == 0: print(f"  ❌ '{keyword}' 관련 리뷰 없음"); return
    print(f"  관련 식당: {filtered['Restaurant'].nunique()}개")

    print(f"\n📊 프로필 생성...")
    profiles, axes_names = build_profiles(filtered, keyword, taste_axes, meta_axes, rules)

    print(f"\n  📋 결과: {len(profiles)}개 식당")
    ...
    print(f"\n📋 완료! {len(profiles)}개 식당 | 맛축 {len(taste_axes)}개 + 메타축 {len(meta_axes)}개 = {len(axes_names)}개")
