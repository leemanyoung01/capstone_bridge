...
def _build_source_restaurant_set(conn, keyword: str, terms: list[str]) -> tuple[set, set]:
    """
    keyword의 source restaurant_id 집합을 두 신호로 구성:
      (1) review_images.image_url에 `/reviews/<keyword>/` (raw 또는 URL-encoded) prefix
      (2) reviews.crawl_keyword 가 keyword/alias 중 하나와 정확히 일치

    반환: (src_by_prefix, src_by_crawl) — 두 집합.
    union이 source ground truth. 둘 다 비면 호출자가 호환 모드 fallback을 결정.
    """
    ...
    # (2) crawl_keyword 정확 일치 (컬럼 미존재 시 빈 집합)
    ...
    return src_by_prefix, src_by_crawl
...
def get_reviews_for_profiler(
    ...
    *,
    ...
    src_by_prefix, src_by_crawl = _build_source_restaurant_set(conn, keyword, terms)
    source_set = src_by_prefix | src_by_crawl
    ...
    for c in candidates:
        ...
        # source boundary 신호 (prefix / crawl_keyword) 자체가 강한 보호 신호.
        # 이 식당은 본문에 keyword가 안 나와도 정당 — density·category 면제.
        ...
        protected = protected_name or protected_crawl or is_prefix_source
