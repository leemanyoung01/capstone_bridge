...
def get_reviews_for_profiler(conn, keyword: str, aliases: Optional[list[str]] = None) -> list[dict]:
    ...
    # --- 교차 오염 방지 필터링 (TF-IDF 방식의 밀도 기반 필터) ---
    noise_names = '샐러드|포케|피자|버거|카페|디저트|베이커리|샌드위치'
    
    # 키워드/별칭 매칭 조건 (이름 검색용)
    name_match_cond = " OR ".join(["rst.name ILIKE %s"] * len(terms))
    name_params = [f"%{t}%" for t in terms]
    
    exclude_cond = ""
    ratio_threshold = 0.02 # 기본 2%
    
    if keyword in ["청국장", "김치찌개", "한상", "칼국수", "아구찜", "조개", "족발", "회"]:
        exclude_cond = f"AND (rst.category IS NULL OR rst.category !~* '샐러드|피자|버거|카페|디저트|베이커리|양식|브런치|포케|다이어트|샌드위치')"
    elif keyword in ["피자", "버거"]:
        # 피자/버거는 사이드 메뉴와 겹치는 경우가 많아 더 엄격한 5% 기준 적용
        ratio_threshold = 0.05
        exclude_cond = f"AND (rst.category IS NULL OR rst.category !~* '한식|중식|일식|해물|찌개|국밥|백반|국수|족발|고기구이|조개|포차|술집')"
        # 이름에도 조개, 포차 등이 있으면 제외 (카테고리 없는 경우 대비)
        exclude_cond += " AND rst.name !~* '조개|창고|포차|횟집|국밥|찌개|식당|회집'"
    elif keyword in ["샐러드"]:
        exclude_cond = f"AND (rst.category IS NULL OR rst.category !~* '찌개|국밥|고기구이|해물|중식|족발|치킨|피자|버거|백반|한상')"

    with conn.cursor() as cur:
        # 비율(Ratio) 필터를 적용한 식당 ID 목록 먼저 추출
        cur.execute(f"""
            SELECT r.restaurant_id
            FROM reviews r
            JOIN restaurants rst ON rst.restaurant_id = r.restaurant_id
            WHERE 1=1 {exclude_cond}
            GROUP BY r.restaurant_id, rst.name
            HAVING 
                ({name_match_cond}) 
                OR (
                    COUNT(*) FILTER (WHERE r.content ILIKE ANY(%s) OR r.menu ILIKE ANY(%s))::float / COUNT(*)::float >= %s
                    AND COUNT(*) FILTER (WHERE r.content ILIKE ANY(%s) OR r.menu ILIKE ANY(%s)) >= 2
                )
        """, name_params + [[f"%{t}%" for t in terms]] * 2 + [ratio_threshold] + [[f"%{t}%" for t in terms]] * 2)
        
        valid_rest_ids = [row['restaurant_id'] for row in cur.fetchall()]
